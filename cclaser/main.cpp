...
void MainLoop(int socket_udp, const struct sockaddr_in &address, struct xv11lidar *laser, pwm *laser_motor);
...
void InitLaserMotor(pwm *motor, const char *motor_pwm_device, int duty_cycle);
...
void MainLoop(int socket_udp, const struct sockaddr_in &address, struct xv11lidar *laser, pwm *laser_motor)
{
	...
	for(counter=0;!g_finish_program && counter<benchs;++counter)
	{
		...
		{
			fprintf(stderr, "cclaser: ReadLaser failed with status %d\n", status);
			break;
		}
	...
	printf("cclaser: avg loop %f seconds\n", seconds_elapsed/counter);
	printf("cclaser: last laser rpm %f\n", packet.laser_speed/64.0);
}
...
int ProcessInput(int argc, char **argv, int *out_port, int *duty_cycle, int *crc_tolerance_pct)
{
	...
	{
		fprintf(stderr, "cclaser: the argument port has to be in range <1, 65535>\n");
		return -1;
	}
	*out_port=port;
	...
	if(duty <= 0 || duty > 100)
	{
		fprintf(stderr, "cclaser: the argument duty_cycle has to be in range <0, 100>\n");
		return -1;
	}
	*duty_cycle=duty;
	...
	if(crc < 0 || crc > 100)
	{
		fprintf(stderr, "cclaser: the argument crc_tolerance_pct has to be in range <0, 100>\n");
		return -1;
	}
...
void Usage()
{
	printf("cclaser tty motor_pwm host port duty_cycle crc_tolerance_pct\n\n");
	printf("examples:\n");
	printf("./cclaser /dev/ttyO4 /sys/class/pwm/pwm-3:0 192.168.0.103 8002 40 10\n");
}
...
void InitLaserMotor(pwm *motor,const char *motor_pwm_device, int duty_cycle)
{
	if(!motor->connect(motor_pwm_device))
		Die("cclaser: laser motor pwm not accessible");

	motor->set_duty_cycle(0);
	motor->set_period(10000000);
	
	motor->set_duty_cycle_pct(duty_cycle);
	motor->set_enable(1);
}
